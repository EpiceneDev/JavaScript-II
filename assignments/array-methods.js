...
let ticketPriceTotal = runners.map(elem => elem.donation).reduce((totalDon, don) => totalDon + don)
...
// Problem 1
//What companies participated?
let gabRunners = []
runners.filter(elem => elem.company_name).forEach(elem => gabRunners.push(elem.company_name))
console.log(gabRunners)

// Problem 2
//Who donated more than $100 to receive a free t-shirt

let freeT = []
runners.filter(person => person.donation >= 100).forEach(elem => freeT.push(elem.first_name + ' ' + elem.shirt_size))

console.log(freeT)


// Problem 3
//Email addresses and companies for followup:
let emailList = runners.map(elem =>elem.email,)
let emailCo = [];
runners.forEach(elem => emailCo.push(`${elem.email} ${elem.company_name}`))
console.log(emailCo);
